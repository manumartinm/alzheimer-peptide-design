...
from .enums import LabelTier
from .features import FeatureComputer, compute_features
...
class Augmenter:
    ...
    def run(
        ...
        *,
        ...
        for _, parent in candidates.iterrows():
            ...
            for aug_idx in range(max(1, cfg.n_augmented_per_sample)):
                ...
                if new_seq == parent_seq:
                    ...
                    continue
                if not self._valid_sequence(new_seq):
        ...
        augmented_df = self.feature_computer.add_columns(augmented_df, sequence_col=sequence_col)
    ...
    def _valid_sequence(self, seq: str) -> bool:
        if not seq or not set(seq).issubset(CANONICAL_AA):
            return False
        return self.config.min_length <= len(seq) <= self.config.max_length
...
def augment_gold_dataframe(
    ...
    *,
    ...
    return Augmenter(config=cfg).run(gold_df, sequence_col=sequence_col)
