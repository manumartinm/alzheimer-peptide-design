...
class FoldSplitter:
    n_splits: int = 5
    seed: int = 42
    holdout_value: str = Split.VAL.value
    ...
    def mark_external_holdout(
        ...
        out = df.copy()
        out["external_test"] = (
            out[split_col].astype(str).str.lower().eq(self.holdout_value).astype(int)
        )
        return out
    ...
    def assign_cluster_folds(
        ...
        n_eff = max(2, min(self.n_splits, max_allowed)) if max_allowed >= 2 else 1
        ...
        splitter = StratifiedGroupKFold(n_splits=n_eff, shuffle=True, random_state=self.seed)
...
def mark_external_holdout(
    df: pd.DataFrame, split_col: str = "split", holdout_value: str = Split.VAL.value
) -> pd.DataFrame:
    return FoldSplitter(holdout_value=holdout_value).mark_external_holdout(df, split_col=split_col)
...
def assign_cluster_folds(
    ...
    return FoldSplitter(n_splits=n_splits, seed=seed).assign_cluster_folds(
        df,
        label_col=label_col,
        cluster_col=cluster_col,
        only_non_holdout=only_non_holdout,
