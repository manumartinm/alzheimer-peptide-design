...
def _dataset_path(cfg: HFExportConfig) -> Path:
    processed = cfg.base_dir / "data" / "processed"
    if cfg.variant == "full":
        return processed / "peptides_bbb_with_augmentation.parquet"
    return processed / "peptides_bbb.parquet"


def _manifest_path(cfg: HFExportConfig) -> Path:
    return cfg.base_dir / "data" / "processed" / "peptides_struct_manifest.parquet"


def _merge_peptides_with_structures(peptides: pd.DataFrame, manifest: pd.DataFrame) -> pd.DataFrame:
    ...
    merged = peptides.merge(struct, on="sequence", how="left", validate="m:1")
    return merged
    ...
    if include_cif:
        ...
        if run_dir.exists():
            try:
                ...
                cif_rel = _relative_cif_path(seq_hash)
            except FileNotFoundError:
                cif_rel = None

    coords_rel = _relative_structure_path(seq_hash) if dst_coords.exists() else None
    return {"structure_coords_path": coords_rel, "structure_cif_path": cif_rel}


def build_hf_release(cfg: HFExportConfig) -> dict[str, object]:
    ...
    peptides_path = _dataset_path(cfg)
    ...
    merged = _merge_peptides_with_structures(peptides, manifest)
    ...
    if cfg.copy_structures:
        ...
        for _, row in merged.iterrows():
            seq_hash = str(row.get("sequence_hash") or sequence_hash(str(row["sequence"]).upper()))
            coords_paths.append(_relative_structure_path(seq_hash))
            cif_paths.append(_relative_cif_path(seq_hash) if cfg.include_cif else None)
    ...
    write_parquet(export_df, parquet_path)
    ...
    (out_root / "README.md").write_text(_render_readme(export_df, stats), encoding="utf-8")
    return {"dataframe": export_df, "stats": stats, "output_dir": out_root}


def _render_readme(df: pd.DataFrame, stats: dict[str, object]) -> str:
    chem_cols = [
        c
        for c in df.columns
        if c
...
Built with `TFG/dataset` (`tfg-bbb-build`, `tfg-bbb-fold`). Structures predicted with Boltz 2.1 via the Boltz API.

## Citation

If you use this dataset, cite the TFG BBB peptide curation pipeline and Boltz structure prediction.
"""
