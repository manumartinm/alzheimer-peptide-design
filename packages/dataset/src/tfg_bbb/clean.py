...
def filter_sequences(
    ...
    allowed = allowed_aa or CANONICAL_AA
    seqs = df[sequence_col].astype(str).str.upper()

    keep_len = seqs.str.len().between(min_length, max_length)
...
def deduplicate_by_identity(
    ...
    assignments = _run_cdhit_or_mmseqs(seqs, threshold)
    if assignments is None:
        assignments = _cluster_python(seqs, threshold)
