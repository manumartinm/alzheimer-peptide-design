...
class ESMTabMLP(nn.Module):
    def __init__(self, d_esm: int, d_tab: int, hidden_dim: int = 256, dropout: float = 0.2):
        super().__init__()
        self.esm_proj = nn.Sequential(nn.LayerNorm(d_esm), nn.Linear(d_esm, hidden_dim // 2), nn.GELU())
        self.tab_proj = nn.Sequential(nn.LayerNorm(d_tab), nn.Linear(d_tab, hidden_dim // 2), nn.ReLU())
