...
def run(args: argparse.Namespace) -> None:
    ...
    if model_type not in CLASSIFIER_MODEL_TYPES:
        raise ValueError(
            f"{model_type} is not a classifier model; use scripts/geo/train.py for evaluation via predict"
        )

    df = pd.read_parquet(args.dataset_path)
    y = df[data_cfg["label_col"]].to_numpy(dtype=int)
    tab = (
        tabular_matrix(df, meta.get("tab_cols", []))
        if exp_cfg["features"].get("use_tabular", False)
        else None
    )
    esm = (
        batch_esm_embeddings(
            df[data_cfg["sequence_col"]].astype(str).tolist(),
    ...
    graphs = (
        [sequence_graph(s) for s in df[data_cfg["sequence_col"]].astype(str).tolist()]
