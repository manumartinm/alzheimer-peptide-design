...
def build_features(df: pd.DataFrame, data_cfg: dict[str, Any], exp_cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    x_3d = feature_matrix_3d(df, data_cfg.get("three_d_columns", [])) if feats_cfg.get("use_3d", False) else None
    graphs = [sequence_graph(s) for s in df[seq_col].astype(str).tolist()] if feats_cfg.get("use_gnn", False) else None
    return {"tab": x_tab, "esm": x_esm, "feat3d": x_3d, "graphs": graphs, "tab_cols": tab_cols, "df": df}
